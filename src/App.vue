...
const tabs = computed(() => [
  ...
  { name: 'friends', path: '/friends', label: t('nav.friends') },
  { name: 'log', path: '/log', label: t('nav.log') },
  { name: 'calendar', path: '/calendar', label: t('nav.calendar') },
])
...
<template>
  <div class="mx-auto max-w-[480px] min-h-screen bg-white relative" style="padding-top: var(--safe-top); padding-bottom: calc(4.5rem + var(--safe-bottom))">
    <!-- App brand: icon + title (clickable, returns to home) -->
    <router-link
      v-if="showLogin"
      to="/"
      class="absolute top-3 left-3 z-30 flex items-center gap-2 no-underline"
      style="margin-top: env(safe-area-inset-top, 0px)"
    >
      <img src="/favicon-32.png" :alt="t('app.brand')" class="w-7 h-7 rounded-lg" />
      <span class="text-[13px] font-semibold text-stone-900 tracking-tight">{{ t('app.brand') }}</span>
    </router-link>

    <!-- Right side: GitHub, locale toggle, account -->
    <div v-if="showLogin" class="absolute top-3 right-3 z-30 flex items-center gap-2" style="margin-top: env(safe-area-inset-top, 0px)">
      <a
        ...
        class="w-7 h-7 flex items-center justify-center text-stone-400 hover:text-stone-700 transition-colors"
      >
      ...
      >
        ...
        <button @click.stop.prevent="handleLogout" class="text-stone-400 hover:text-stone-600 text-xs" :title="t('app.logout')">
